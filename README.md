...
tweets <- tweets %>% mutate(party = map_chr(text, detect_party)) %>%filter(party != "Other")   
tidy_tweets <- tweets %>%    #function to each tweet and add a new parrty column, filter out tweets that have no parties mentioned especially the ones I have mentioned in the code above 
